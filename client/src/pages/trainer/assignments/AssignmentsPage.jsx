...
import { FiPlus, FiClock, FiCheckSquare, FiSearch, FiFilter, FiMoreVertical, FiFileText } from 'react-icons/fi';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Create, track, and grade student assignments across your courses.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-[#1e2035] p-5 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Needs Grading</p>
        ...
        <div className="bg-white dark:bg-[#1e2035] p-5 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">328</h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1e2035] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-[#1a1c2e] border-b border-gray-200 dark:border-white/10">
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Assignment Details</th>
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden sm:table-cell">Course</th>
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Submissions</th>
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">Due Date</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-white/5">
              {assignments.map(assignment => (
                <tr key={assignment.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-white line-clamp-1">{assignment.title}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 sm:hidden mt-0.5 line-clamp-1">{assignment.course}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 hidden sm:table-cell">
                    <span className="text-sm text-gray-600 dark:text-gray-300">{assignment.course}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-gray-700 dark:text-gray-300 font-medium">{assignment.submitted} / {assignment.total}</span>
                        <span className="text-gray-500 dark:text-gray-500">{Math.round((assignment.submitted / assignment.total) * 100)}%</span>
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2.5 py-1 text-xs font-semibold rounded-full
                      ${assignment.status === 'Active' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : ''}
                      ${assignment.status === 'Grading' ? 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400' : ''}
                      ${assignment.status === 'Completed' ? 'bg-gray-100 text-gray-700 dark:bg-gray-500/20 dark:text-gray-400' : ''}
                    `}>
                      {assignment.status}
                    </span>
                  </td>
          ...
          {/* Empty State placeholder if list was empty */}
          {assignments.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
               ...
               <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-sm">You haven't created any assignments yet, or none match your current filters.</p>
